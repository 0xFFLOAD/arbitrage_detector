...
void BitstampFetcher::run() {
    while (running_) {
        // helper for fetching the REST ticker across all symbols; we need it in
        // both the BNB polling branch and the websocket-maintenance code.
        ...
        // poll-only mode for BNB; the websocket channel is silent and the
        // API is more reliable via REST.  Do an immediate call before anything
        // else so that the price is seeded quickly and the detector won't
        // complain about a missing value on startup.
        if (symbol_ == Symbol::BNBUSDC) {
            // initial seed outside of the timer loop
            double initial = restTicker(symbol_);
            if (initial > 0.0) {
                storage_.updatePrice(Exchange::Bitstamp, symbol_, Price::fromDouble(initial));
                LOG("Bitstamp: " << to_string(symbol_) << " = $" << initial << " (rest)");
            }
            ...
            // nothing else to clean up in the BNB case
            break;
        }
