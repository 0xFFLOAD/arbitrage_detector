...
void BitstampFetcher::run() {
    ...
    bool attempted_usdc = (channel.find("usdc") != std::string::npos);
    bool ws_mode = !channel.empty();
    ...
    while (running_) {
        ...
        try {
            ...
            if (attempted_usdc && !coingecko_id.empty()) {
                std::cerr << "Bitstamp USDC websocket failed, switching to Coingecko fallback" << std::endl;
                ws_mode = false;
                channel.clear();
                continue;
            }
